...
#define ENV_CONFIG_BASE                                         \
    ...
    "log=(enabled=true,file_max=10M,remove=%s),session_max=%d," \
...
recover_and_verify(uint32_t backup_index, uint32_t workload_iteration)
{
    ...
    char backup_dir[PATH_MAX], buf[PATH_MAX], fname[64], kname[64], verify_dir[PATH_MAX],
      lr_source_dir[PATH_MAX], live_restore_cfg[PATH_MAX + 200];
    char ts_string[WT_TS_HEX_STRING_SIZE];
    bool fatal;
    ...
    if (backup_index == 0) {
        ...
        if (live_restore_backups) {
            // Emulate a live restore by moving the database into a live restore source directory.
            // We'll then live restore it back into the original verify directory.
            testutil_snprintf(lr_source_dir, sizeof(lr_source_dir), "%s_lr_source", verify_dir);
            testutil_snprintf(live_restore_cfg, sizeof(live_restore_cfg),
              "live_restore=(enabled=true,path=%s,threads_max=4)", lr_source_dir);
            testutil_move(verify_dir, lr_source_dir);
         ...
         */
        // Ignore this. I think I'm doing the wrong thing in the test and need to investigate
        // if (live_restore_backups) {
        //     /* Instead of copying the backup into the verify dir create an empty verify dir and use the backup as the live restore source. */
        //     testutil_mkdir(verify_dir);
        //     testutil_snprintf(live_restore_cfg, sizeof(live_restore_cfg),
        //       "verbose=(fileops:2),live_restore=(enabled=true,path=%s,threads_max=4)", backup_dir);
        //     testutil_wiredtiger_open(
